...
#ifdef OPLUS_FEATURE_CAMERA_COMMON
#define DEVICE_VERSION_IMX586     "imx586"
...
#if USE_BURST_MODE
static kal_uint16 imx586_table_write_cmos_sensor(kal_uint16 *para, kal_uint32 len);
#endif
...
static struct SENSOR_WINSIZE_INFO_STRUCT imgsensor_winsize_info[10] = {
	...
	{8000, 6000, 0, 400, 8000, 5200, 4000, 2600,  0,   0, 4000, 2600,  0,  0, 4000, 2600}, /* normal video */
	{8000, 6000, 0, 440, 8000, 5120, 2000, 1280,  0,  76, 2000, 1128,  0,  0, 2000, 1128}, /* hs_video */
	...
	{8000, 6000, 0, 440, 8000, 5120, 2000, 1280,  0,  76, 2000, 1128,  0,  0, 2000, 1128}, /* custom4 */
...
static struct SET_PD_BLOCK_INFO_T imgsensor_pd_info_binning =
{
	...
	.i4SubBlkW  = 8,
	.i4SubBlkH  = 2,
	.i4PosL = {{20,13},{18,15},{22,17},{24,19},{20,21},{18,23},{22,25},{24,27}},
	.i4PosR = {{19,13},{17,15},{21,17},{23,19},{19,21},{17,23},{21,25},{23,27}},
	.i4BlockNumX = 496,
	.i4BlockNumY = 186,
	.iMirrorFlip = 0,
	.i4Crop = { {0, 0}, {0, 0}, {0, 200}, {0, 0}, {0, 372}, {0, 0}, {80,420}, {0, 0}, {0, 0}, {0, 0} },
};
...
{
	kal_uint16 idx = 0, addr_qsc = 0xC90, sensor_addr = 0x7F00;
	kal_uint16 addr_lrc = 0x1620;

	for (idx = 0; idx <2304; idx++) {
		addr_qsc = 0xC90 + idx;
		sensor_addr = 0x7F00 + idx;
		imx586_QSC_setting[2*idx] = sensor_addr;
		imx586_QSC_setting[2*idx + 1] = read_cmos_eeprom_8(addr_qsc, SLAVEADDR);
	}

	for (idx = 0; idx < 192; idx++) {
		addr_lrc = 0x1620 + idx;
		sensor_addr = 0x7510 + idx;
		imx586_LRC_setting[2*idx] = sensor_addr;
		imx586_LRC_setting[2*idx + 1] = read_cmos_eeprom_8(addr_lrc, SLAVEADDR);
	}
	for (idx = 192; idx < 384; idx++) {
		addr_lrc = 0x1620 + idx;
		sensor_addr = 0x7600 + idx - 192;
		imx586_LRC_setting[2*idx] = sensor_addr;
		imx586_LRC_setting[2*idx + 1] = read_cmos_eeprom_8(addr_lrc, SLAVEADDR);
	}
}
...
static void write_sensor_LRC(void)
{
	...
	for (idx = 0; idx <192; idx++) {
		addr_lrc = 0x7510 + idx;
		write_cmos_sensor_8(addr_lrc, imx586_LRC_setting[2*idx + 1]);
	}
	for (idx = 192; idx < 384; idx++) {
		addr_lrc = 0x7600 + idx - 192;
		write_cmos_sensor_8(addr_lrc, imx586_LRC_setting[2*idx + 1]);
	}
	#endif
}
...
static void feedback_awbgain(kal_uint32 r_gain, kal_uint32 b_gain)
{
	...
	imx586_feedback_awbgain[3] = (((r_gain*100) / 512) - (r_gain_int * 100)) * 2;
	imx586_feedback_awbgain[5] = b_gain_int;
	imx586_feedback_awbgain[7] = (((b_gain * 100) / 512) - (b_gain_int * 100)) * 2;
...
static void custom5_setting(void)
{
	...
	#if USE_BURST_MODE
	imx586_table_write_cmos_sensor(imx586_custom5_setting,
	...
	write_cmos_sensor_8(0x0305, 0x04);
	write_cmos_sensor_8(0x0306, 0x01);
	write_cmos_sensor_8(0x0307, 0x14);
	write_cmos_sensor_8(0x030B, 0x01);
	write_cmos_sensor_8(0x030D, 0x0C);
	write_cmos_sensor_8(0x030E, 0x02);
	write_cmos_sensor_8(0x030F, 0xE4);
	write_cmos_sensor_8(0x0310, 0x01);
	/*Other Setting*/
	write_cmos_sensor_8(0x3620, 0x00);
	write_cmos_sensor_8(0x3621, 0x00);
	...
	write_cmos_sensor_8(0x3FFF, 0x6C);
	/*Integration Setting*/
	write_cmos_sensor_8(0x0202, 0x0A);
	write_cmos_sensor_8(0x0203, 0xC4);
	write_cmos_sensor_8(0x0224, 0x01);
	write_cmos_sensor_8(0x0225, 0xF4);
	write_cmos_sensor_8(0x3FE0, 0x01);
	write_cmos_sensor_8(0x3FE1, 0xF4);
		...
		do {
			*sensor_id = ((read_cmos_sensor_8(0x0016) << 8)
			...
			if (*sensor_id == imgsensor_info.sensor_id) {
				...
				read_sensor_Cali();
				LOG_INF("IMX586_module_id=%d\n",imgsensor_info.module_id);
				if(deviceInfo_register_value == 0x00){
					Oplusimgsensor_Registdeviceinfo("Cam_r0", DEVICE_VERSION_IMX586,
									imgsensor_info.module_id);
				...
				if (is_project(19011) || is_project(19301)) {
					pr_info("19011/19301: imx586 set mirrorflip\n");
			  ...
			  MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	...
	spin_unlock(&imgsensor_drv_lock);
	if (!qsc_flag) {
		LOG_INF("custom3 write_sensor_QSC Start\n");
		write_sensor_QSC();
		LOG_INF("custom3 write_sensor_QSC End\n");
		qsc_flag = 1;
	}
	custom3_setting();
	set_mirror_flip(imgsensor.mirror);
			   ...
			   MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	...
	#ifdef OPLUS_FEATURE_CAMERA_COMMON
	if (is_project(19357) || is_project(19358) || is_project(19359) || is_project(19354))
	{
